...
app.get('/gameList', async (req, res) => {
  try {
      // Obtener los roles de forma asíncrona usando la función getRoles
      const results = await db_a_s.getGames();
...
app.delete('/delete-game/:id', async (req, res) => {
  ...
  try {
    const result = await db_a_s.deleteGame(id); // Call deleteUser function
...
app.put('/update-game/:id', async (req, res) => {
  ...
  // Validación básica
  if (price === undefined || !description || !imageUrl) {
    return res.status(400).json({ error: 'Price, description, and imageUrl are required' });
  }

  try {
    const result = await db_a_s.modifyGame(price, description, imageUrl, id);
