...
const db = mysql.createPool({
  ...
  database: process.env.DB_NAME_A,
});
...
const updateCredentials = async (signInEmail, accessCredentials) => {
  const query = process.env.UPDATE_CREDENTIALS;
  const plain = signInEmail + accessCredentials;
  const hash = await bcrypt.hash(plain, 10);
  const [result] = await db.query(query, [hash, signInEmail]);
  return result;
};
